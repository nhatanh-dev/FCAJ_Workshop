...
---
{{% notice warning %}}
⚠️ **Lưu ý:** Các thông tin dưới đây là bản đề xuất chi tiết cho hệ thống SmartInvoice Shield, được xây dựng dựa trên yêu cầu thực tế của doanh nghiệp và quy định pháp luật hiện hành.
{{% /notice %}}

Tại phần này, báo cáo tóm tắt các nội dung và giải pháp kiến trúc trong dự án mà nhóm **đã và đang** triển khai.

# SmartInvoice Shield
## Hệ thống Quản trị & Rà soát Rủi ro Hóa đơn Điện tử trên nền tảng AWS

### 1. Tóm tắt điều hành
SmartInvoice Shield là một hệ thống SaaS (Software as a Service) được thiết kế chuyên biệt nhằm cung cấp giải pháp toàn diện cho các doanh nghiệp Việt Nam trong việc quản trị và rà soát rủi ro hóa đơn điện tử. Nền tảng tự động hóa quá trình trích xuất dữ liệu từ các định dạng hóa đơn (XML, PDF, Ảnh) thông qua AI, đồng thời thực thi bộ quy tắc kiểm tra tuân thủ 3 lớp (Cấu trúc, Chữ ký số, Nghiệp vụ) dựa trên Nghị định 123/2020/NĐ-CP. Bằng việc tận dụng sức mạnh của các dịch vụ AWS Serverless và Managed Services (Elastic Beanstalk, ECS Fargate, S3, RDS, Cognito, SQS), SmartInvoice Shield mang lại trải nghiệm xử lý tự động khối lượng dữ liệu lớn với độ trễ thấp, đảm bảo tính khả dụng cao (Multi-AZ), tối ưu chi phí vận hành và bảo mật tuyệt đối.
...
- **Minh bạch & Cảnh báo:** Hệ thống đánh giá rủi ro theo 4 cấp độ trực quan (Xanh/Vàng/Cam/Đỏ), giúp kế toán trưởng đưa ra quyết định phê duyệt nhanh chóng.

*Lợi ích và hoàn vốn đầu tư (ROI)*
Hệ thống cam kết giúp doanh nghiệp cắt giảm 90% thời gian xử lý hóa đơn và ngăn chặn 100% rủi ro pháp lý trước khi khai thuế. Với mô hình Event-Driven và phân bổ tài nguyên AWS thông minh (Fargate Spot Scale-to-Zero, EC2 Burstable), chi phí hạ tầng hàng tháng được tối ưu ở mức cực thấp (~63 USD/tháng) cho một môi trường Production Multi-AZ hoàn chỉnh. Thời gian hoàn vốn dự kiến rất ngắn nhờ tiết kiệm trực tiếp chi phí nhân sự và ngăn ngừa triệt để các khoản phạt vi phạm hành chính.
...
*Thiết kế thành phần*
- **Tầng Frontend (Edge):** Ứng dụng React TS + Vite cung cấp giao diện quản lý (Dashboard, Danh sách hóa đơn, Khám nghiệm rủi ro trực quan).
- **Tầng Backend (Core):** 14 API Controllers viết bằng C# .NET 9 thực thi logic phân quyền, workflow phê duyệt và bộ quy tắc kiểm tra pháp lý.
- **Tầng Async (Xử lý nền):** Các Background Workers đọc message từ SQS để phân tích hóa đơn qua AI và đối soát danh sách đen của Tổng cục Thuế.
...
4. **Triển khai Frontend & Domain:** Triển khai mã nguồn React lên AWS Amplify, thiết lập SSL/TLS qua ACM và định tuyến DNS bằng Route 53.
...
- **Tuần 1-4 (Foundation & Core Features):** Thiết kế Database Schema, xây dựng các API cốt lõi, tích hợp Amazon Cognito và quy trình Upload S3.
- **Tuần 5-8 (AI Integration & Testing):** Cấu hình Amazon SQS, triển khai mô hình PaddleOCR/VietOCR lên ECS, lập trình quy tắc kiểm tra tuân thủ 3 lớp (3-layer validation).
- **Tuần 9-12 (Deployment & QA):** Hoàn thiện Dashboard, cấu hình Audit Logs, triển khai toàn hệ thống lên AWS Production. Thực hiện kiểm thử chịu tải (Load testing) và bảo mật.
...
- Application Load Balancer: ~16,00 USD
- AWS Elastic Beanstalk (2 x t3.micro EC2): ~15,00 USD
- Amazon RDS PostgreSQL (db.t3.micro - Multi-AZ): ~30,00 USD
- Amazon S3 & Dữ liệu truyền tải: ~2,00 USD (5GB lưu trữ + Lifecycle to Glacier)
- AWS Amplify & Amazon Cognito: 0 USD (Nằm trong Free Tier)
- ECS Fargate Spot (AI OCR): Trả phí linh hoạt theo phần nghìn giây khi có hóa đơn (Pay-As-You-Go).
*Tổng chi phí ước tính:* ~63,00 USD/tháng cho môi trường Production sẵn sàng phục vụ doanh nghiệp.
